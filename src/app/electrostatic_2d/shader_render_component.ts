import { type GpuRenderTexture, type GpuMesh, create_square_mesh, GpuUniform, GpuCamera2D } from "../../renderers/common.ts";
import { GpuGrid } from "./grid.ts";
import shader_render_component_wgsl from "./shader_render_component.wgsl?raw";

export type DataMode = "voltage" | "Ex" | "Ey";
...
export class ShaderRenderComponent {
  ...
  create_pass(
    ...
    const get_buffer = () => {
      switch (data_mode) {
      case "voltage": return grid.v_field;
      case "Ex": return grid.ex_field;
      case "Ey": return grid.ey_field;
      }
    };
    const data = get_buffer();
    ...
    const render_pipeline = this.get_render_pipeline(data_mode);
