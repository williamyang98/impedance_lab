...
export abstract class NdarrayView {
  ...
  export_as_numpy_bytecode(writer: NdarrayWriter) {
    ...
    // data
    const total_elems = this.shape.reduce((a,b) => a*b, 1);
    const bytes_per_elem = get_dtype_size(this.dtype);
    const total_data_bytes = total_elems*bytes_per_elem;
    ...
    const values_view = new T(u8_buf.buffer, u8_buf.byteOffset+write_offset, total_elems);
    const flattened_view = this.flatten();
    for (let i = 0; i < total_elems; i++) {
      values_view[i] = flattened_view.get([i]);
    }
  }
}
...
export class NdarrayViewReverse extends NdarrayView {
  ...
  override get dtype(): NdarrayType {
...
export class Ndarray extends NdarrayView {
  ...
  static own_from_view(view: NdarrayView): Ndarray {
    const arr = Ndarray.create_zeros(view.shape, view.dtype);
