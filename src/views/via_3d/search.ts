...
export async function search_parameters(
  ...
  const search_function = async (value: number): Promise<SearchResult> => {
    ...
    const stackup_grid = new StackupGrid(executor.gpu_device, stackup, grid_builder_config, profiler);
    profiler.end();
    ...
    const measurement = await executor.run(stackup_grid, toast, profiler);
    profiler.end();
