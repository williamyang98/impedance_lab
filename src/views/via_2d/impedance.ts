...
import type { StackupGrid, ViaBarrelInfo } from "./stackup_to_grid.ts";
...
export type InductanceMethod =
  "solid_fw_grover" |
  "solid_goldfarb_pucel" |
  "tube_dc_vt_morgan" |
  "solid_wu" |
  "tube_sapongin_prokopenko" |
  "tube_thin_sapongin_prokopenko";

export function calculate_via_inductance(via: ViaBarrelInfo, method: InductanceMethod): number {
  const mu_0 = Math.PI*4e-7;
  switch (method) {
    case "solid_fw_grover": {
    ...
    case "solid_wu": {
      // https://upcommons.upc.edu/server/api/core/bitstreams/928a71c0-b0a3-49ad-bf73-6f7a02ad2609/content
      // Equation 9 - Solid cylindrical conductor (Wu et al)
      const h = via.height;
      const r = via.outer_diameter/2;
      const a0 = Math.sqrt(1 + h**2/r**2);
      const a1 = Math.sqrt(1 + r**2/h**2);
      return h*mu_0/(2*Math.PI)*(Math.log(1/r+a0) - a1 + r/h + 1/4);
    }
    case "tube_sapongin_prokopenko": {
      // https://upcommons.upc.edu/server/api/core/bitstreams/928a71c0-b0a3-49ad-bf73-6f7a02ad2609/content
      // Equation 13 - Infinitely long tubular conductors (Sapongin and Prokopenko)
      const h = via.height;
      const r1 = via.inner_diameter/2;
      const r2 = via.outer_diameter/2;
      const a0 = r1/r2;
    ...
    case "tube_thin_sapongin_prokopenko": {
      // https://upcommons.upc.edu/server/api/core/bitstreams/928a71c0-b0a3-49ad-bf73-6f7a02ad2609/content
      // Equation 14 - Infinitely long thin-wall tubular conductors (Sapongin and Prokopenko)
      const h = via.height;
      const r1 = via.inner_diameter/2;
      const r2 = via.outer_diameter/2;
      const R = Math.max(h,r1,r2)*10; // some wierd virtual cylinder we need???
      ...
      const t2 = t*t;
      const t4 = t2*t2;
      return h*mu_0/(4*Math.PI)*(t + t2/6 - t4/30 + 2*Math.log(R/r2));
    }
  }
}
