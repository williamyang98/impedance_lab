...
#[wasm_bindgen]
pub unsafe fn electrostatic_solver(
    v: &mut [f32], e: &mut [f32],
    dx: &[f32], dy: &[f32],
    ...
    let mut dx_mid = vec![0.0; nx-1];
    let mut dy_mid = vec![0.0; ny-1];
    for i in 0..nx-1 {
    ...
    let mut dxy_norm = vec![0.0; (nx-1)*(ny-1)];
    for y in 0..ny-1 {
        for x in 0..nx-1 {
            let i = x + y*(nx-1);
...
#[wasm_bindgen]
pub unsafe fn calculate_homogenous_energy(
    e: &[f32],
    dx: &[f32], dy: &[f32],
) -> f32 {
    let nx: usize = dx.len();
...
#[wasm_bindgen]
pub unsafe fn calculate_inhomogenous_energy(
    e: &[f32], er: &[f32],
    dx: &[f32], dy: &[f32],
) -> f32 {
    let nx: usize = dx.len();
